  ...
  public static Pattern robotsPatternToRegexp(String pattern)
  {
    ...
    for (int pos = 0; pos < pattern.length(); ++pos) {
      ...
      if (ch == '\\') {
        ...
        if (quote_buf.length() > 0)
        {
          // The quoted character buffer is not empty, so add them before adding
          // the wildcard matcher
          regexp.append("\\Q").append(quote_buf).append("\\E");
          quote_buf = new StringBuilder();
        }
        ...
        // A $ at the end of the pattern indicates that the path should end here in order to match
